...
static const png_byte png_signature[8] = { 0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a };
...
bool PNG_HandleIHDR(const png_byte *data, png_image_t *image)
{
...
void PNG_Filter(png_byte *out, png_byte filter, const png_byte *in, const png_byte *lastline, png_ulong rowbytes, png_ulong bpp)
{
	png_ulong		i;
...
void PNG_Unfilter(png_byte *out, png_byte filter, const png_byte *lastline, png_ulong rowbytes, png_ulong bpp)
{
	png_ulong	i;
...
bool PNG_Pack(png_byte *out, png_ulong *size, png_ulong maxsize, png_byte *data, int width, int height, int bytedepth)
{
	z_stream		zdata;
	png_ulong			rowbytes;
	png_ulong			y;
	const png_byte		*lastline, *source;
	// Storage for filter type and filtered row
	png_byte			workline[(MAX_PNG_WIDTH * MAX_PNG_DEPTH) + 1];

	// Number of bytes per row
	rowbytes = width * bytedepth;

	memset(&zdata, 0, sizeof(z_stream));
	if(deflateInit(&zdata, Z_BEST_COMPRESSION) != Z_OK)
	{
	...
	for(y = 0; y < height; y++)
	{
		...
		workline[0] = (png_byte)PNG_FILTER_VALUE_PAETH;
		PNG_Filter(workline + 1, (png_byte)PNG_FILTER_VALUE_PAETH, source, lastline, rowbytes, bytedepth);
...
bool PNG_Unpack(const png_byte *data, const png_ulong datasize, png_image_t *image)
{
	png_ulong		rowbytes, zerror, y;
	png_byte		filter;
	z_stream	zdata;
	png_byte		*lastline, *out;
...
bool PNG_Load(const png_byte *data, png_ulong datasize, png_image_t *image)
{
	bool			moredata;
	const png_byte		*next;
	png_byte			*workspace, *work;
	png_ulong			length, type, crc, totallength;
	...
	workspace = (png_byte *)Z_Malloc(datasize, TAG_TEMP_PNG, qfalse);
	work = workspace;
	totallength = 0;
	...
	while(moredata)
	{
		length = BigLong(*(png_ulong *)data);
		data += sizeof(png_ulong);

		type = BigLong(*(png_ulong *)data);
		const png_byte *crcbase = data;
		data += sizeof(png_ulong);

		// CRC checksum location
		next = data + length + sizeof(png_ulong);

		// CRC checksum includes header field
		crc = crc32(0, crcbase, length + sizeof(png_ulong));
		if(crc != (png_ulong)BigLong(*(png_ulong *)(next - 4)))
		{
			if(image->data)
			{
				Z_Free(image->data);
		...
		case PNG_IHDR:
			...
			image->data = (png_byte *)Z_Malloc(image->width * image->height * image->bytedepth, TAG_TEMP_PNG, qfalse);
			break;
		case PNG_IDAT:
...
bool PNG_OutputChunk(fileHandle_t fp, png_ulong type, png_byte *data, png_ulong size)
{
	png_ulong	crc, little, outcount;
	...
	little = BigLong(type);
	crc = crc32(0, (png_byte *)&little, sizeof(little));
	outcount += FS_Write(&little, sizeof(little), fp);
...
bool PNG_Save(const char *name, png_byte *data, int width, int height, int bytedepth)
{
	...
	if(!PNG_OutputChunk(fp, PNG_IHDR, (png_byte *)&png_header, sizeof(png_header)))
	{
		FS_FCloseFile(fp);
		return(false);
	}
	// Create and output the copyright info
 	if(!PNG_OutputChunk(fp, PNG_tEXt, (png_byte *)png_copyright, sizeof(png_copyright)))
	{
		FS_FCloseFile(fp);
		return(false);
	}
	...
	work = (png_byte *)Z_Malloc(maxsize, TAG_TEMP_PNG, qtrue);	// fixme: optimise to qfalse sometime - ok?
	...
	if(!PNG_OutputChunk(fp, PNG_IDAT, (png_byte *)work, size))
	{
		Z_Free(work);
		FS_FCloseFile(fp);
		return(false);
...
void PNG_ConvertTo32(png_image_t *image)
{
	...
	temp = (png_byte *)Z_Malloc(image->width * image->height * 4, TAG_TEMP_PNG, qtrue);
	old = image->data;
	old2 = old;
	image->data = temp;
	image->bytedepth = 4;
