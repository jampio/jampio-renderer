#include <map>
#include <jampio/common/CvarSystem.h>
...
===============
*/
void GL_TextureMode(const glconfig_t& glConfig, CvarSystem& cvars, const char *string) {
	int		i;
	image_t	*glt;
	...
	{
		cvars.Set(r_ext_texture_filter_anisotropic->name(), va("%f",glConfig.maxTextureFilterAnisotropy) );
	}
...
static float R_BytesPerTex(const glconfig_t& glConfig, int format)
{
	switch ( format ) {
	case 1:
		//"I    " 
...
===============
*/
float R_SumOfUsedImages(const glconfig_t& glConfig, const trGlobals_t& tr, qboolean bUseFormat) 
{	
	int	total = 0;
	image_t *pImage;
	...
	while ( (pImage = R_Images_GetNextIteration()) != NULL)
	{
		if ( pImage->frameUsed == tr.frameCount- 1 ) {//it has already been advanced for the next frame, so...
			if (bUseFormat)
			{
				float  bytePerTex = R_BytesPerTex(glConfig, pImage->internalFormat);
...
===============
*/
void R_ImageList_f(const glconfig_t& glConfig) {
	int		i=0;
	image_t	*image;
	int		texels=0;
	float	texBytes = 0.0f;
	...
	while ( (image = R_Images_GetNextIteration()) != NULL)
	{
		...
		texBytes += image->width*image->height * R_BytesPerTex(glConfig, image->internalFormat);
...
================
*/
void R_LightScaleTexture(const glconfig_t& glConfig, unsigned *in, int inwidth, int inheight, qboolean only_gamma )
{
	if ( only_gamma )
	{
...
static void Upload32(const glconfig_t& glConfig, 
						 unsigned *data, 
						 GLenum format,
						 qboolean mipmap, 
						 qboolean picmip, 
						 ...
						 unsigned short *pUploadWidth, unsigned short *pUploadHeight, bool bRectangle = false )
{
	...
	if (format == GL_RGBA)
	{
		...
		*pUploadHeight = height;
		...
		R_LightScaleTexture (glConfig, data, width, height, (qboolean)!mipmap );
...
static void GL_ResetBinds(glstate_t& glState)
{
...
// (avoid using ri.xxxx stuff here in case running on dedicated)
//
void R_Images_DeleteLightMaps(glstate_t &glState)
{
...
//
qboolean RE_RegisterImages_LevelLoadEnd(glstate_t& glState)
{
...
================
*/
image_t *R_CreateImage(const glconfig_t& glConfig, glstate_t& glState, const char *name, const byte *pic, int width, int height, 
...
//	memset(image,0,sizeof(*image));	// qtrue above does this 
	...
	Upload32( glConfig, (unsigned *)pic,	format,
...
//rwwRMG - added
void R_CreateAutomapImage(const glconfig_t &glConfig, glstate_t& glState, const char *name, const byte *pic, int width, int height, 
					   qboolean mipmap, qboolean allowPicmip, qboolean allowTC, int glWrapClampMode ) 
{
	R_CreateImage(glConfig, glState, name, pic, width, height, GL_RGBA, mipmap, allowPicmip, allowTC, glWrapClampMode);
}
...
static void LoadJPG( const char *filename, unsigned char **pic, int *width, int *height ) {
  ...
  jpeg_stdio_src(&cinfo, (FILE *)fbuffer);
...
==============
*/
image_t	*R_FindImageFile(const glconfig_t& glConfig, glstate_t& glState, const char *name, qboolean mipmap, qboolean allowPicmip, qboolean allowTC, int glWrapClampMode ) {
	image_t	*image;
	int		width, height;
	byte	*pic;
	GLenum	format;
	...
	image = R_CreateImage(glConfig, glState, name, pic, width, height, format, mipmap, allowPicmip, allowTC, glWrapClampMode );
	Z_Free( pic );
	return image;
}
...
static void R_CreateDlightImage(trGlobals_t& tr, const glconfig_t& glConfig, glstate_t& glState) 
{
	int		width, height;
	byte	*pic;
	GLenum	format;
	...
	if (pic)
	{                                    
		tr.dlightImage = R_CreateImage(glConfig, glState, "*dlight", pic, width, height, GL_RGBA, qfalse, qfalse, qfalse, GL_CLAMP );
		Z_Free(pic);
	}
	else
	{	// if we dont get a successful load
		...
		tr.dlightImage = R_CreateImage(glConfig, glState, "*dlight", (byte *)data, DLIGHT_SIZE, DLIGHT_SIZE, GL_RGBA, qfalse, qfalse, qfalse, GL_CLAMP );
	}
}
...
=================
*/
void R_InitFogTable(trGlobals_t& tr) {
	int		i;
	float	d;
	float	exp;
...
================
*/
float R_FogFactor(const trGlobals_t& tr, float s, float t) {
	float	d;
...
#define	FOG_S	256
#define	FOG_T	32
static void R_CreateFogImage(trGlobals_t& tr, const glconfig_t& glConfig, glstate_t& glState) {
	int		x,y;
	byte	*data;
	float	g;
	float	d;
	...
	for (x=0 ; x<FOG_S ; x++) {
		for (y=0 ; y<FOG_T ; y++) {
			d = R_FogFactor(tr, ( x + 0.5f ) / FOG_S, ( y + 0.5f ) / FOG_T );
	...
	// what we want.
	tr.fogImage = R_CreateImage(glConfig, glState, "*fog", (byte *)data, FOG_S, FOG_T, GL_RGBA, qfalse, qfalse, qfalse, GL_CLAMP );
...
static void R_CreateDefaultImage(trGlobals_t& tr, const glconfig_t& glConfig, glstate_t& glState) {
	int		x;
	...
	tr.defaultImage = R_CreateImage(glConfig, glState, "*default", (byte *)data, DEFAULT_SIZE, DEFAULT_SIZE, GL_RGBA, qtrue, qfalse, qfalse, GL_REPEAT );
}
...
==================
*/
void R_CreateBuiltinImages(trGlobals_t& tr, CvarSystem& cvars, const glconfig_t& glConfig, glstate_t& glState) {
	int		x,y;
	byte	data[DEFAULT_SIZE][DEFAULT_SIZE][4];

	R_CreateDefaultImage(tr, glConfig, glState);

	// we use a solid white image instead of disabling texturing
	Com_Memset( data, 255, sizeof( data ) );
	tr.whiteImage = R_CreateImage(glConfig, glState, "*white", (byte *)data, 8, 8, GL_RGBA, qfalse, qfalse, qfalse, GL_REPEAT);

	tr.screenImage = R_CreateImage(glConfig, glState, "*screen", (byte *)data, 8, 8, GL_RGBA, qfalse, qfalse, qfalse, GL_REPEAT );
	...
	{
		cvars.Set(r_DynamicGlowWidth->name(), glConfig.vidWidth);
	}
	if ( r_DynamicGlowHeight->integer() > glConfig.vidHeight  )
	{
		cvars.Set(r_DynamicGlowHeight->name(), glConfig.vidHeight);
	}
	...
	tr.identityLightImage = R_CreateImage(glConfig, glState, "*identityLight", (byte *)data, 8, 8, GL_RGBA, qfalse, qfalse, qfalse, GL_REPEAT);

	for(x=0;x<NUM_SCRATCH_IMAGES;x++) {
		// scratchimage is usually used for cinematic drawing
		tr.scratchImage[x] = R_CreateImage(glConfig, glState, va("*scratch%d",x), (byte *)data, DEFAULT_SIZE, DEFAULT_SIZE, GL_RGBA, qfalse, qtrue, qfalse, GL_CLAMP);
	}

	R_CreateDlightImage(tr, glConfig, glState);

	R_CreateFogImage(tr, glConfig, glState);
}
...
===============
*/
void R_SetColorMappings(trGlobals_t& tr, const glconfig_t& glConfig, CvarSystem& cvars) {
	int		i, j;
	float	g;
	int		inf;
	int		shift;
	...
	if ( r_intensity->value() < 1.0f ) {
		cvars.Set(r_intensity->name(), 1);
	}

	if ( r_gamma->value() < 0.5f ) {
		cvars.Set(r_gamma->name(), "0.5");
	} else if ( r_gamma->value() > 3.0f ) {
		cvars.Set(r_gamma->name(), "3.0" );
	}
...
===============
*/
void R_InitImages(trGlobals_t& tr, const glconfig_t& glConfig, glstate_t& glState, CvarSystem& cvars) {
	// build brightness translation tables
	R_SetColorMappings(tr, glConfig, cvars);

	// create default texture and white texture
	R_CreateBuiltinImages(tr, cvars, glConfig, glState);
}
...
//
void R_DeleteTextures(glstate_t& glState) {
	R_Images_Clear();	
	GL_ResetBinds(glState);
}
...
qhandle_t RE_RegisterIndividualSkin(const trGlobals_t& tr, const char *name , qhandle_t hSkin) 
{
	skin_t		*skin;
	skinSurface_t	*surf;
	char		*text, *text_p;
...
qhandle_t RE_RegisterSkin(trGlobals_t& tr, const char *name) {
	qhandle_t	hSkin;
	skin_t		*skin;
...
*/
	...
	if ( RE_SplitSkins(name, skinhead, skintorso, skinlower ) )
	{//three part
		hSkin = RE_RegisterIndividualSkin(tr, skinhead, hSkin);
		if (hSkin)
		{
			hSkin = RE_RegisterIndividualSkin(tr, skintorso, hSkin);
			if (hSkin)
			{
				hSkin = RE_RegisterIndividualSkin(tr, skinlower, hSkin);
			}
		}
	}
	else
	{//single skin
		hSkin = RE_RegisterIndividualSkin(tr, name, hSkin);
	}
	return(hSkin);
}
...
qhandle_t RE_RegisterServerSkin(trGlobals_t& tr, const char *name) {
	qhandle_t r;
	...
	{ //If the client is running then we can go straight into the normal registerskin func
		return RE_RegisterSkin(tr, name);
	}

	gServerSkinHack = true;
	r = RE_RegisterSkin(tr, name);
...
===============
*/
void R_InitSkins(trGlobals_t& tr) {
	skin_t		*skin;
...
===============
*/
skin_t *R_GetSkinByHandle(const trGlobals_t& tr, qhandle_t hSkin) {
...
===============
*/
void R_SkinList_f(const trGlobals_t& tr) {
	int			i, j;
	skin_t		*skin;
